...
public class AlbumCommentDAO {
	...
	public static boolean selectComments(int albumId, JSONArray comments) { 
		JSONObject comment;
		rs = null;
		...
		try {
			...
			String query = "SELECT album_comments.id, album_comments.user_id, user_name, first_name, last_name, comment, album_comments.date_created, album_comments.date_updated FROM album_comments "
					+ "INNER JOIN users ON users.user_id = album_comments.user_id "
					+ "WHERE album_id = ?";
	...
	public static boolean createComment(int userId, int albumId, String comment) {
		queryStatus = 0;
		try {
			conn = new DBConnection().getConnection();
			String query = "INSERT INTO album_comments (user_id, album_id, comment) VALUES (?, ?, ?)";
			prepStat = conn.prepareStatement(query);
	...
	public static boolean updateComment(int id, String comment) {
		...
		try {
			conn = new DBConnection().getConnection();
			String query = "UPDATE album_comments SET comment = ? WHERE id = ?";
			prepStat = conn.prepareStatement(query);
	...
	public static boolean deleteComment(int id) {
		...
		try {
			conn = new DBConnection().getConnection();
			String query = "DELETE FROM album_comments WHERE id = ?";
			prepStat = conn.prepareStatement(query);
